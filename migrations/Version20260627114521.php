 ...
 */
final class Version20260627114521 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__account AS SELECT id, email, password, created_at, updated_at, user_id FROM account');
        $this->addSql('DROP TABLE account');
        $this->addSql('CREATE TABLE account (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, email VARCHAR(255) NOT NULL, password VARCHAR(255) DEFAULT NULL, created_at DATETIME NOT NULL, updated_at DATETIME DEFAULT NULL, user_id INTEGER DEFAULT NULL, oauth_provider VARCHAR(32) DEFAULT NULL, oauth_id VARCHAR(255) DEFAULT NULL, CONSTRAINT FK_7D3656A4A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) ON UPDATE NO ACTION ON DELETE NO ACTION NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO account (id, email, password, created_at, updated_at, user_id) SELECT id, email, password, created_at, updated_at, user_id FROM __temp__account');
